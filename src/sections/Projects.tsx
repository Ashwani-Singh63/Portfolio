    ...
    <section id="projects" className="py-32 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col items-center mb-24">
          ...
          <h2 className="text-7xl font-black text-center tracking-tighter text-slate-900 dark:text-white">Recent Projects</h2>
        </div>
          ...
          {EXPERIENCES[0].projects?.map((project, idx) => (
            <SectionReveal key={project.id}>
              <div className={`group grid lg:grid-cols-12 gap-12 items-center ${idx % 2 === 0 ? '' : 'lg:flex-row-reverse'}`}>
                ...
                <div className={`lg:col-span-5 relative z-10 ${idx % 2 === 0 ? 'lg:order-2 lg:-ml-24' : 'lg:order-1 lg:-mr-24'}`}>
                  <GlassCard className="p-12 lg:p-16 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8">
                       <span className="text-slate-100 dark:text-slate-900 text-8xl font-black select-none">0{idx + 1}</span>
                    </div>
                    <div className="relative">
                      <h3 className="text-4xl font-black mb-4 group-hover:text-primary-600 transition-colors leading-tight text-slate-900 dark:text-white">{project.title}</h3>
